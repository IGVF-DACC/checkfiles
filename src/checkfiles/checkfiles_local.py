...
def file_validation(input_file_path, validation_record: file.FileValidationRecord, submitted_md5sum, content_type, file_format_type, assembly, tabular_file_schema_path, max_tabular_file_errors, reference_file_path, onlist_skip):
    ...
    if file_format == 'bam':
        bam_check_result = bam_pysam_check(input_file_path, content_type)
        ...
        tabular_file_check_error = tabular_file_check(
            file_format, content_type, input_file_path, schema_path=tabular_file_schema_path, max_error=max_tabular_file_errors, allow_additional_fields=True)
